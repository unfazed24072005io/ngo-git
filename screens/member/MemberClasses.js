// screens/member/MemberClasses.js
...
export default function MemberClasses({ navigation }) {
  ...
  const renderKey = `member-classes-${counter}`;
  ...
  const translations = {
    ...
    registrationFee: t('onlineClasses.registrationFee') || 'Registration Fee',
    ...
    securePayment: t('onlineClasses.securePayment') || '🔒 Secure payment via Razorpay',
    payAndRegisterBtn: 'Pay ₹{amount} & Register',
  ...
  const initiateRegistration = (classItem) => {
    ...
    if (classItem.capacity && classItem.registeredCount >= classItem.capacity) {
      Alert.alert(translations.classFull, translations.classFullMsg);
      return;
    }
  ...
  const auth = getAuthInstance(); // ✅ ADD THIS
    ...
    try {
      ...
      Alert.alert(translations.success, translations.registerSuccess.replace('{title}', classItem.title));
    ...
    try {
      ...
      if (paymentResult.success) {
        ...
        if (verificationResult.success) {
          ...
          Alert.alert(translations.paymentFailed, translations.paymentVerificationFailed);
        }
      } else {
        Alert.alert(translations.paymentFailed, paymentResult.error || translations.somethingWentWrong);
      }
    } catch (error) {
      ...
      <View style={styles.modalContainer}>
        <View style={styles.paymentModalContent}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>{translations.registrationFeeTitle}</Text>
            <TouchableOpacity 
              onPress={() => {
                if (!paymentLoading) setShowPaymentModal(false);
              }}
          ...
          {classToRegister && (
            <>
              <View style={styles.paymentInfo}>
                ...
                <Text style={styles.paymentFeeLabel}>{translations.registrationFeeLabel}</Text>
                <Text style={styles.paymentFeeAmount}>₹{classToRegister.fee}</Text>
                <Text style={styles.paymentDescription}>
                  {translations.feeDescription}
                </Text>
              </View>
              ...
              >
                {paymentLoading ? (
                  ...
                  <Text style={styles.payFeeButtonText}>
                    {translations.payAndRegisterBtn.replace('{amount}', classToRegister.fee)}
                  </Text>
                )}
              </TouchableOpacity>
            ...
            {selectedClass && (
              <>
                ...
                {selectedClass.fee && selectedClass.fee > 0 && (
                  <View style={styles.detailSection}>
                    <Text style={styles.detailLabel}>{translations.registrationFee}</Text>
                  ...
                  >
                    ...
                    <Text style={styles.registerText}>
                      {registeredClasses.includes(selectedClass.id) ? translations.registered :
                       selectedClass.registeredCount >= selectedClass.capacity ? translations.full :
                       selectedClass.fee && selectedClass.fee > 0 ? 
                         translations.payAndRegister.replace('{amount}', selectedClass.fee) : 
