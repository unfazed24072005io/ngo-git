// screens/member/MemberIDCard.js
...
export default function MemberIDCard({ navigation }) {
  const { t, counter } = useLanguage();
  
  // Force re-render when language changes
  const renderKey = `member-idcard-${counter}`;
  ...
  const translations = {
    ...
    // Card
    ngoMember: t('idcard.ngoMember') || 'NGO MEMBER',
    ...
    shareTitle: t('idcard.shareTitle') || 'My NGO ID Card',
    shareMessage: t('idcard.shareMessage') || 'NGO Member ID Card\n\nName: {name}\nID: {id}\nEmail: {email}\nPhone: {phone}',
  };
  ...
  const handleShare = async () => {
    try {
      const shareMessage = translations.shareMessage
        .replace('{name}', userData?.fullName || userData?.name || 'Member')
        .replace('{id}', cardId)
        ...
        <View style={styles.cardContainer}>
          <View style={styles.card}>
            <View style={styles.cardHeader}>
              ...
              <Text style={styles.cardTitle}>{translations.ngoMember}</Text>
