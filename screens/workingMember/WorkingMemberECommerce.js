...
export default function WorkingMemberECommerce({ navigation }) {
  ...
  const renderKey = `working-ecommerce-${counter}`;
  ...
  const translations = {
    ...
    books: t('ecommerce.books') || 'Books',
    clothing: t('ecommerce.clothing') || 'Clothing',
    accessories: t('ecommerce.accessories') || 'Accessories',
    food: t('home.food') || 'Food',
    other: t('common.other') || 'Other',
    
    // Product Card
    productDetails: t('ecommerce.productDetails') || 'Product Details',
    outOfStock: t('ecommerce.outOfStock') || 'Out of Stock',
    add: t('common.add') || 'Add',
    bulk: t('ecommerce.bulk') || 'BULK',
    product: t('ecommerce.product') || 'Product',
    
    // Cart
    addedToCart: t('ecommerce.addedToCart') || 'Added to Cart',
    addedToCartMsg: t('ecommerce.addedToCartMsg') || '{name} added to cart',
    cartEmpty: t('ecommerce.cartEmpty') || 'Cart Empty',
    pleaseAddItems: t('ecommerce.pleaseAddItems') || 'Please add items to your cart',
    
    // Empty State
    noProductsFound: t('ecommerce.noProducts') || 'No products found',
    checkBackLater: t('ecommerce.checkBackLater') || 'Check back later for new items',
    
    // Loading
    loadingProducts: t('ecommerce.loadingProducts') || 'Loading products...',
  };
  ...
  const navigateToCart = () => {
    ...
    navigation.navigate('WorkingMemberCart', { cart });
  };
  ...
  const ProductCard = ({ product }) => {
    ...
    const handleAddToCart = () => {
      if (product.stock === 0) {
        Alert.alert(translations.outOfStock, translations.outOfStock);
        return;
      }
      addToCart(product);
    };
      ...
      <View style={styles.productCard}>
        ...
        >
          ...
          <Text style={styles.productCardDescription} numberOfLines={2}>
            {product.shortDescription || product.category || translations.product}
          </Text>
          {showWholesale && hasWholesalePrice ? (
  ...
  return (
    <View style={styles.container} key={renderKey}>
      ...
      {/* Cart Floating Button - Navigates to Cart Screen */}
      <TouchableOpacity 
